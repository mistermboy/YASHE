...
CodeMirror.defineMode("shex", function(config, parserConfig) {
 ...
 // var ATPNAME_NS = "@[a-zA-Z]*:";
  ...
  function tokenBase(stream, state) {
   // console.log({stream:stream,state:state})
    function nextToken() {
      var consumed = null;
      if (state.inLiteral) {
    ...
    if (!tokenOb.quotePos || tokenOb.quotePos == "end") {
      ...
      while (state.stack.length > 0 && token && state.OK && !finished) {
        ...
        //console.log({topSymbol:topSymbol,tokenOb:tokenOb,state:state})
        if(topSymbol=='shexDoC')console.log('asdas')

       
        if(tokenOb.cat=="ATPNAME_LN"){
          let spRef=tokenOb.text.slice(1,tokenOb.text.length);
          let defShapes = Object.keys(state.defShapes);
          let isDefined = defShapes.filter(dfs=>dfs==spRef).length > 0
          if(!isDefined){
            state.OK = false;
            recordFailurePos();
            state.errorMsg = "Shape " + spRef + " is not defined";
          }
        }
        if(topSymbol=='shapeExprLabel'){
          console.log({tokenOb:tokenOb,stack:state})
